...
- [xtream-tv.apk](https://github.com/admin-xtinex/xtream/releases/download/v2.7.0/xtream-tv.apk) for Android TV (landscape, TV launcher)
- [xtream-mobile.apk](https://github.com/admin-xtinex/xtream/releases/download/v2.7.0/xtream-mobile.apk) for phones
...
**Cursor** is on Home, Bookmarks, History, and the page bar. It is hidden while a video is playing. Arrows move the pointer and OK clicks it. Push the pointer against the top or bottom edge to scroll whatever is under it, a sidebar or the page. **Remote** puts the arrows back on the buttons. Up from the top of a page returns to Home, and holding Up gets there from anywhere, including after a video played inside the page. Type a search or an `http`/`https` address. Bookmarks and recent pages stay on the device.

**Video quality.** Players are pushed to the best level the stream offers, never under 1080p when 1080p exists, up to 4K when the site streams it and the TV can decode it. Change it under Options → Video Quality (Best, 1080p, 720p, 480p, 360p, or Auto to let the site decide); the choice is remembered. The TV app loads the desktop version of sites, since phone versions start low.

**Security.** Pages run on the device's own Android System WebView, so keep that updated; Home warns when it is about a year or more out of date. The app never fetches router, Wi-Fi or TV-local addresses on a page's behalf, only lets other apps open plain `http`/`https` links, keeps page debugging to debug builds, leaves browsing data out of device backups, and restarts a page cleanly if its engine crashes.
