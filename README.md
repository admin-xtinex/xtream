...
- [xtream-tv.apk](https://github.com/admin-xtinex/xtream/releases/download/v2.0.0/xtream-tv.apk) for Android TV (landscape, TV launcher)
- [xtream-mobile.apk](https://github.com/admin-xtinex/xtream/releases/download/v2.0.0/xtream-mobile.apk) for phones
...
The phone app turns with the device. **Rotate** locks it to the other direction. When a video starts, the phone goes landscape and the picture fills the screen. Back leaves that view. The TV app stays landscape. The install file includes the Gecko browser engine, so it does not depend on the system WebView. **Blocking** uses that engine’s tracking protection. **Video** saves only a direct video file. Pages are loaded the same way Chrome loads them, so a site is less likely to swap the player for an install prompt.
